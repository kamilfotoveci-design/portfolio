const state={lang:(()=>{try{return localStorage.getItem('portfolio-lang')==='en'?'en':'cs'}catch{return'cs'}})()};
...
document.querySelectorAll('.lang').forEach(button=>button.addEventListener('click',()=>{state.lang=button.dataset.lang;localStorage.setItem('portfolio-lang',state.lang);document.documentElement.lang=state.lang;document.querySelectorAll('.lang').forEach(item=>{const active=item.dataset.lang===state.lang;item.classList.toggle('is-active',active);item.setAttribute('aria-pressed',active)});translations().forEach(item=>{item.innerHTML=item.dataset[state.lang]});labelTargets().forEach(item=>{item.setAttribute('aria-label',item.dataset[state.lang+'Label'])});altTargets().forEach(item=>{item.alt=item.dataset[state.lang+'Alt']});const contactLink=document.querySelector('.contact-cta');if(contactLink)contactLink.href=state.lang==='en'?'mailto:kami.hortik@gmail.com?subject=New%20project%20%E2%80%94%20Kamil%20Hort%C3%ADk&body=Hi%20Kamil,%0A%0AProject:%0A%0AWhat%20I%20need:%0A%0ATimeline:%0A%0ABudget:%0A%0AReferences:%0A':'mailto:kami.hortik@gmail.com?subject=Nov%C3%BD%20projekt%20%E2%80%94%20Kamil%20Hort%C3%ADk&body=Ahoj%20Kamile,%0A%0AProjekt:%0A%0ACo%20pot%C5%99ebuji:%0A%0ATerm%C3%ADn:%0A%0ARozpo%C4%8Det:%0A%0AReference:%0A';window.dispatchEvent(new CustomEvent('portfolio:language',{detail:{lang:state.lang}}));if(activeSceneId&&sceneAnnouncer&&sceneNames[activeSceneId])sceneAnnouncer.textContent=sceneNames[activeSceneId][state.lang]}));
...
if(state.lang==='en')document.querySelector('.lang[data-lang=en]')?.click();
